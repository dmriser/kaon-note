...
\subsection{Inclusive Electron Cross Section}
As an additional validation step for a different analysis, the authors undertook the measurement of the inclusive electron scattering cross section in the resonance region as a function of $W$ and $Q^2$.  A short description of that work is provided here as a form of validation of our good run list, as well as our electron identification procedure. \\

A simple two dimensional $W$ and $Q^2$ binning scheme was used to count inclusive events.  Then, acceptance corrections were applied using events generated with `keppel\_rad`.  Finally, bin centering corrections and radiative corrections were applied using the generators `keppel\_norad` and `keppel\_rad`.  The resulting cross sections agree well with the model by Keppel.

\begin{figure}
	\begin{center}
		\includegraphics[width=6cm]{image/plots/inclusive/cross-section-sect2-slice3.png}		
		\includegraphics[width=6cm]{image/plots/inclusive/cross-section-ratio-sect2-slice3.png}

		\caption{Left: The cross section measured from $E1-F$ using our electron identification and good run list.  The pink line corresponds to the model prediction by Cynthia Keppel's inclusive resonance parametrization.  Right: The ratio of our calculation to the model values shown on the left figure.}
	\end{center}		
\end{figure}
